...
def extract_vacancy_info(vacancy: str, llm_handler: LLMHandler):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future1 = executor.submit(extract_vacancy_details, vacancy,  llm_handler, model="gpt-4.1-mini")
        future2 = executor.submit(extract_vacancy_role, vacancy,  llm_handler, model="gpt-4.1-mini")
        ...
        future5 = executor.submit(extract_vacancy_rate, vacancy, llm_handler, model="gpt-4.1-nano")
    ...
    combined_reasoning = '\n\n'.join(reasoning for reasoning in [vacancy_reasoning_1, vacancy_reasoning_2, vacancy_reasoning_3, vacancy_reasoning_4, vacancy_reasoning_5] if reasoning)
    ...
    extracted_data.update({k: v
                           for d in [extracted_data1, extracted_data2, extracted_data3, extracted_data4, extracted_data5]
                           for k, v in d.items()
