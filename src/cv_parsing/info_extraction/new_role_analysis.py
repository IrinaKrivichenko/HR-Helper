...
from typing import List, Optional, Literal, Type, TypeVar
from pydantic import BaseModel, Field, create_model

T = TypeVar('T', bound=BaseModel)

def create_role_analysis_model(existing_roles: List[str]) -> Type[BaseModel]:
    """
    Dynamically creates RoleAnalysis model with Literal type for overlaps_with field.
    Args:
        existing_roles: List of predefined roles for overlaps_with field.
    Returns:
        Pydantic model RoleAnalysis with Literal type for overlaps_with.
    """
    # Create Literal type for overlaps_with field
    OverlapsType = Literal[*existing_roles] if existing_roles else str
    return create_model(
        'RoleAnalysis',
        ...
        overlaps_with=(Optional[OverlapsType], Field(
            default=None,
            description="Existing role it overlaps with (if recommendation is Overlaps)"
        )),
    )
...
def _analyze_single_role(proposed_role: str,
    ...
    # Dynamically create RoleAnalysis model
    RoleAnalysis = create_role_analysis_model(existing_roles)
