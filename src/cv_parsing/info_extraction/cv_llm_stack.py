...
def extract_cv_stack(cv: str, llm_handler: LLMHandler, model="gpt-4.1-nano", add_tokens_info: bool = False):
    """
    Extract CV technology stack using Schema-Guided Reasoning approach.
    """
    ...
    prompt = [
        ...
        },
        {
            ...
            "content": (
                ...
                f"{{\n"
                f'  "reasoning_steps": ["Found Python in 2 projects", "JavaScript mentioned in skills only"],\n'
                f'  "technologies": [\n'
                f'    {{"name": "Python", "score": 4}},\n'
                f'    {{"name": "JavaScript", "score": 3}}\n'
                f'  ]\n'
                f"}}\n\n"
                f"**Resume:**\n{cv}"
            )
    ...
    if not response:
        return {"Stack": "", "Cost_of_Stack_CV_extraction": 0}

    print(response)  # Debug print (как в оригинале)
    ...
    try:
        ...
        if not cleaned_response:
            return _fallback_cv_stack_parse(response, token_data, add_tokens_info)

        parsed_data = json.loads(cleaned_response)

        # Validate with Pydantic
        cv_analysis = CVStackAnalysis(**parsed_data)
    ...
    # Format stack string (sorted by score descending)
...
def _fallback_cv_stack_parse(response: str, token_data: dict, add_tokens_info: bool) -> dict:
    """
    Fallback parser for cases when JSON parsing fails.
    """
    result = {}
